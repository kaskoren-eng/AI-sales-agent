...
**tenants.settings key claims:** `voice_engine` (VOICE) · `functions_enabled` (VOICE) · `whatsapp_templates` (VOICE) · `toll_fraud` (VOICE) · `reminders` (VOICE) · `crm_sync` (VOICE, Workstream B) · `businessProfile` (shared) · `zadarma` (VOICE) · `monday` (shared) · `airtable` (shared) · `flows` (pre-existing, shared) · `billing_provider` (reserved, Workstream D) · `agent_persona` (**CLAIMED 2026-08-16, VOICE-owned** — agent name/gender/company/FAQ/voice. Operator-only through the generic settings escape hatch, because a wrong TTS `voiceId` makes Cartesia and ElevenLabs return a *silent stream* rather than an error. Tenants edit the CONTENT half through the typed route `PUT /settings/agent-persona`, which has no `tts` field and is `.strict()`. See `src/modules/channels/voice-livekit/persona.ts`) · `ui_locale` (**reserved** for dashboard interface language — never `language`). · `handoff` (VOICE — human-handoff owner contact + notify channels, migration 0017) · `callbacks` (**CLAIMED 2026-09-03, VOICE-owned · WIDENED 2026-09-04** — the follow-up ladder. `enabled`, `maxAttempts` (ceiling raised 3 → **5**), `proactiveWeekday`/`proactiveFriday` calling hours, `disconnectedDelayMinutes`, `dayParts` (morning/afternoon anchors + split), and **`ladders`** — the tenant's own rungs per kind, because Koren's requirement is that each client decides what follow-ups its agent does. What a tenant still CANNOT set: a rung's `window` (`honored` belongs to an hour the lead named), a rung's `channel` (the worker still dials unconditionally), rung 1 of the `explicit` ladder (the lead's own time), the hard floor (23:00–07:00, Shabbat, Israeli holidays), or opt-out. Tenant-writable through the typed route **`PUT /settings/callbacks`**, which is `.strict()` and returns the RESOLVED config so silent clamps are visible. Resolver: `src/modules/channels/voice-livekit/tools/callback-settings.ts`) · New keys → add here in the same commit.
...
- `billing` — **usage metering write path (Phase 5a: meters run silently — no enforcement, no UI).** `usage_events` is an append-only ledger; `usage_periods` is a CACHE of it, and drift is always resolved TOWARDS the ledger. **Billable unit is the MINUTE** (Koren, 2026-09-01 — packages are capped by call minutes ONLY, never by lead count; the per-package minute figures are still TBD in `docs/gtm/pricing-model.md`). `plans.includedLeads` is superseded by `plans.includedMinutes` and is frozen on periods that were priced on leads; `meterCall` writes duration seconds into `usage_periods.secondsUsed`. `meterLead` still records one unit per lead and is now a COUNT signal, not a billing unit (`pricing.ts`, milli-agorot, versioned rate card whose numbers are **unverified list prices**). Idempotency is the `(tenant_id, kind, dedupe_key)` unique index, not application logic. `meterLead`/`meterCall` never throw — safe only because both units are rebuildable by `npm run usage:reconcile`. A new `insert(leads)` site that neither meters nor carries a `usage-metering: exempt` marker fails `metering-coverage.test.ts`.
...
- `leads/stop-signals.ts` + `stop-guard.ts` — **the follow-up STOP guardrail (2026-09-04).** Three tiers, not a boolean: HARD STOP ("אל תתקשרו אליי", "STOP") → `leads.status='opted_out'`, permanent and cross-channel; SOFT STOP ("לא מעוניין") → `leads.followup_stopped_at`, ends the ladder and is lifted the moment he writes back; CONTINUE ("תתקשר מחר") → timing, not refusal, and the whole reason the ladder exists. Detection is a deterministic Hebrew/English phrase list FIRST (a legal obligation must not depend on OpenAI being up) and an LLM classifier second; a hard phrase short-circuits the classifier, and a classifier outage falls back to the lists rather than stopping everybody. Wired into `message-processor.worker.ts` (every inbound WhatsApp/email message, BEFORE the terminal-status return and the lead-intake flow) and `end-call.tool.ts` (`opt_out` → hard, `not_interested` → soft).
