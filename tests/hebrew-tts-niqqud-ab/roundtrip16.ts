/**
 * Round-trip verification for round 16 — synth -> 8kHz phone band -> Soniox -> what came back?
 *
 ...
 *
 * A PASS is necessary, never sufficient. index-round16.html is the other half.
 *
 *   npx tsx tests/hebrew-tts-niqqud-ab/roundtrip16.ts
 *   npx tsx tests/hebrew-tts-niqqud-ab/roundtrip16.ts br      # one card
 */
...
async function main(): Promise<void> {
  ...
  const data = JSON.parse(await readFile(join(HERE, 'round16.json'), 'utf-8')) as { cards: Card[] };

  // Merged rather than overwritten: a partial run must not erase an earlier card's readings.
  const heardPath = join(HERE, 'round16-heard.json');
  ...
  console.log('wrote round16-heard.json — re-run build_round16_page.py to fold it into the page.');
  process.exit(0);
}
