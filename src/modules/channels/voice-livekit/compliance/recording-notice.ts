...
/**
 * Played to a caller who reached a number we cannot attribute to an active tenant, just before the
 * room is disconnected.
 *
 * Same player, same broadcast voice, different sentence — generated by
 * `scripts/generate-system-announcements.mjs`. If the file is absent the player logs and returns
 * null; the call is still refused, because the hang-up is the security behaviour and the audio is
 * only the courtesy.
 */
export const NOT_IN_SERVICE_PATH = 'assets/not-in-service.wav';
