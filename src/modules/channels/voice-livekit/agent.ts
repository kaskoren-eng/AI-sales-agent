...
class ClickScalesAgent extends voice.Agent {
   ...
   *     nothing in our stack does, so it went straight to the TTS (Cartesia, the engine at the
   *     time) and was read out.
   *
   ...
   * reply start rather than at first token, which is the whole point — the TTS begins
...
function timeFirstAudioFrame(
  ...
  // the CONSUMER asks — so it timed when LiveKit played the frame, not when the TTS produced it,
...
export default defineAgent({
  ...
  entry: async (ctx: JobContext) => {
    ...
    // `scripts/generate-system-announcements.mjs`, which calls Cartesia's HTTP API directly and
    // therefore still needs a CARTESIA_API_KEY even though the agent itself speaks DeepDub) the
    // player logs and
